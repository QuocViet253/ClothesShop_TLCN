...
export class ProductsingleComponent extends BaseComponent implements OnInit {
  ...
  getProduct() {
    this.productService.getList().subscribe(
      (res) => {
        this.product = res.filter((x: any) => x.product_id == this.p_id)[0];
        // this.listSize = this.product.size.toString().split(',');
        this.productService.getImage().subscribe(
          (res) => {
            this.listPImage = res.filter((x: any) => x.product_id == this.p_id);
            this.productService.getDetail().subscribe(
              (res) => {
                this.listPDetail = res.filter((x: any) => x.product_id == this.p_id);
              }
            )
          }
        )
  ...
  getListCommentByProduct() {
    ...
    this.commentService.getList().subscribe(
      (res) => {
        // this.lstCommentByAccount = res.filter((x: any) => x.account_id == acc_id && x.product_id == this.p_id);
        this.lstCommentByAccount = res;
        this.lstCommentByAccount.forEach((x: any) => {
          x.user_name = this.listAccount.filter((a: any) => a.account_id == x.account_id)[0].user_name ?? "";
        })
      }
    );
    // if (localStorage.getItem('UserInfo')) {
  ...
  getlstProductRecommend() {
    this.productService.getList().subscribe(
      (res) => {
        if (res.length > 0) {
          this.lstProductRecommend = res.slice(0,4);
        }
      }
    )
  }
