...
export class ShopComponent extends BaseComponent implements OnInit {
  ...
  getlistProductFilter() {
    ...
    this.productService.getList().subscribe(
      (res) => {
        this.listProductFilter = res.filter((x: any) => x.category_id == this.cate_id);
        ...
        this.productService.getColor().subscribe(
          (res) => {
            ...
            var arrColor = [...new Set(res.filter((x: any) => arrProdId.includes(x.product_id)).map((c: any) => c.color))];
